...
with tab1:
    ...
    st.markdown("Search arXiv for papers on nanotwinned electrodeposited FCC copper. Customize the query, download PDFs, and generate metadata in multiple formats for NER analysis or cloud studies.")
    ...
    def query_arxiv(query, categories, max_results, start_year, end_year, exact_phrases=[]):
        try:
            ...
            formatted_terms = []
            synonyms = {
                "nanotwin": ["nanotwinned", "twin boundary", "coherent twin boundary", "incoherent twin boundary", "twin boundary spacing"],
                "twin spacing": ["twin boundary spacing", "itb spacing", "tb spacing", "ctb spacing", "twin thickness", "spacing"],
                "electrodeposition": ["electroplating", "deposition", "direct current electroplating", "pulsed electrodeposition"],
                "copper": ["cu", "(111) cu", "fcc cu", "cu substrate"],
    ...
    def save_to_sqlite(df, db_file="nanotwin_papers.db"):
        try:
    ...
    # Save to Parquet
    def save_to_parquet(df, parquet_file="copper_papers_metadata.parquet"):
        try:
            ...
            if not papers:
                ...
                **Suggestions to find more papers:**
                ...
                if "SQLite (.db)" in output_formats:
                    sqlite_status = save_to_sqlite(df.drop(columns=["abstract_highlighted"]), "nanotwin_papers.db")
...
with tab2:
    ...
    patterns = [
        [{"TEXT": {"REGEX": r"^\d+(\.\d+)?$"}}, {"LOWER": {"IN": ["ma/cm²", "ma/cm2", "a/cm²", "a/cm2", "a/m²", "a/m2", "a/dm²", "a/dm2"]}}],  # Updated for CURRENT_DENSITY
        ...
        [{"TEXT": {"REGEX": r"^\d+(\.\d+)?$"}}, {"LOWER": {"IN": ["µm", "micrometer", "micrometers"]}}, {"LOWER": {"IN": ["grain", "size"]}, "OP": "?" }],  # Micrometers
            ...
            for match_id, start, end in matches:
                ...
                if unit and value is not None:
                    if unit.lower() in ["a/cm²", "a/cm2"]:
                        ...
                        value *= 0.1  # Convert A/m² to mA/cm² (1 A/m² = 0.1 mA/cm²)
                        unit = "mA/cm²"
                    elif unit.lower() in ["a/dm²", "a/dm2"]:
                        value *= 10  # Convert A/dm² to mA/cm² (1 A/dm² = 10 mA/cm²)
                        unit = "mA/cm²"
                    elif unit.lower() == "m":
                        unit = "mol/L"
        ...
        if not pdf_dir_input or not metadata_csv:
            ...
            if df is None or df.empty:
                ...
                **Next Steps**:
                ...
                - Use CSV/JSON/SQLite/Parquet for further analysis or cloud studies.
                """)
